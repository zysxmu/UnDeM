...
save_list=["/data/fhdmi_class/train/class1/moire",
           "/data/fhdmi_class/train/class2/moire",
           "/data/fhdmi_class/train/class3/moire",
           "/data/fhdmi_class/train/class4/moire"] # fix to your path

clear_folder="/data/fhdmi_patches/train/clear" # fix to your path
moire_folder="/data/fhdmi_patches/train/moire/" # fix to your path
score_list = {}
for i in save_list:
    if os.path.exists(i):
        pass
...
def extract_signle(opt):
    ...
    sort_result = sorted(score_list.items(), key=lambda x: (x[1][2]),
                         reverse=False)  # 先按照名字排序，再按照同名的排序score，排序结果是同名的patch都相连

    class_1 = sort_result[:19964]  # f small, c small
    sort_result = sort_result[19964:]

    sort_result = sorted(sort_result, key=lambda x: (x[1][3]), reverse=False)
    class_2 = sort_result[:19964]  # f small, c big
    class_3 = sort_result[19964:19964*2]  # f big, c big
    class_4 = sort_result[19964*2:]  # f big, c small

    for i in range(0, len(class_1)):
        shutil.copy(osp.join(moire_folder, class_1[i][0]), osp.join(save_list[0], class_1[i][0]))
        shutil.copy(osp.join(clear_folder, class_1[i][0]), osp.join(save_list[4], class_1[i][0]))
    for i in range(0, len(class_2)):
        shutil.copy(osp.join(moire_folder, class_2[i][0]), osp.join(save_list[1], class_2[i][0]))
        shutil.copy(osp.join(clear_folder, class_2[i][0]), osp.join(save_list[5], class_2[i][0]))
    for i in range(0, len(class_3)):
        shutil.copy(osp.join(moire_folder, class_3[i][0]), osp.join(save_list[2], class_3[i][0]))
        shutil.copy(osp.join(clear_folder, class_3[i][0]), osp.join(save_list[6], class_3[i][0]))
    for i in range(0, len(class_4)):
        shutil.copy(osp.join(moire_folder, class_4[i][0]), osp.join(save_list[3], class_4[i][0]))
        shutil.copy(osp.join(clear_folder, class_4[i][0]), osp.join(save_list[7], class_4[i][0]))
